...
function startServer (opts) {
  ...
  var plotlyBrowserify = browserify();
  var plotlyMiddleware;
  var code = [];
  if (!opts.plotlySrc) {
    plotlyBrowserify.require(plotlyPath, {expose: 'plotly.js'});
    plotlyBrowserify.add(toStream('window.Plotly = require("plotly.js")'));
    plotlyMiddleware = createMiddleware(plotlyBrowserify);
  }
  ...
  return budo(path.join(__dirname, 'dummy.js'), {
    ...
    middleware: [
      function (req, res, next) {
        if (plotlyBrowserify && /plotly\.js$/.test(req.url)) {
          plotlyMiddleware(req, res);
        } else {
          next();
        }
      },
    ...
    defaultIndex: function (params, req) {
      ...
      if (plotlyMiddleware) {
        stream = stream.pipe(injectScripts(['/plotly.js']));
      }
