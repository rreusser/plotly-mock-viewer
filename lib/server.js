...
function startServer (opts) {
  ...
  function fetchMathJax (match) {
    console.log('match:', match);
    return fs.readFileSync(require.resolve(path.join('plotly.js/dist/extras/mathjax/', match[1].replace(/\?.*/, ''))), 'utf8')
  }
  ...
  return budo(path.join(__dirname, 'dummy.js'), {
    ...
    middleware: [
      function (req, res, next) {
        var match;
        if ((match = req.url.match(/^\/mathjax\/(.*)/))) {
          res.end(fetchMathJax(match))
        } else {
          next()
        }
      }, function (req, res, next) {
